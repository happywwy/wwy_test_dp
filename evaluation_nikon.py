...
for ind, line in enumerate(nikon_review):
    line = line.split('##')
    ...
    for feature in features:
        ...
        if ' ' not in feature[0]:
            if feature[0] in [row[0] for row in pos_sentence[597+ind]]:
                indice = [row[0] for row in pos_sentence[597+ind]].index(feature[0])
                if pos_sentence[597+ind][indice][1] in vb:
                    count_verb += 1
                    include = False
                   
            else:
                for term in [row[0] for row in pos_sentence[597+ind]]:
                    indice = [row[0] for row in pos_sentence[597+ind]].index(term)
                    if wordnet_lemmatizer.lemmatize(term,'v') == feature[0] and pos_sentence[597+ind][indice][1] in vb:
                        count_verb += 1
                        include = False
            ...
            for word in words:
                if word in [row[0] for row in pos_sentence[597+ind]]:
                    indice = [row[0] for row in pos_sentence[597+ind]].index(word)
                    if pos_sentence[597+ind][indice][1] in vb:
                        count_verb += 1
                        include = False
                        break
                    ...
                    for term in [row[0] for row in pos_sentence[597+ind]]:
                        if wordnet_lemmatizer.lemmatize(term, 'v') == word:
                            indice = [row[0] for row in pos_sentence[597+ind]].index(term)
                            if pos_sentence[597+ind][indice][1] in vb:
                                count_verb += 1
                                include = False
        ...
        if wordnet_lemmatizer.lemmatize(feature[0]) not in nikon_feature and include == True:
            nikon_feature.append(wordnet_lemmatizer.lemmatize(feature[0]))

nikon_phrase = [] 
phrase_count = 0 
phrase_set = []          
for item in nikon_feature:
    if ' ' in item:
        phrase_count += 1
        phrase_set.append(item)
        item = item.split(' ')
        for word in item:
            if word not in nikon_phrase:
                nikon_phrase.append(word)
...
for line in nikon_predict:
    if line != '':
        for item in line:
            item = wordnet_lemmatizer.lemmatize(item)
            if item not in nikon_output:
                nikon_output.append(item)
'''
...
for item in nikon_output:
    if item not in nikon_feature and item not in nikon_phrase:
        confusion[1][0] += 1
        print item,nikon_freq[wordnet_lemmatizer.lemmatize(item)],
