...
for ind, line in enumerate(canon_review):
    line = line.split('##')
    ...
    for feature in features:
        ...
        if ' ' not in feature[0]:
            if feature[0] in [row[0] for row in pos_sentence[ind]]:
                indice = [row[0] for row in pos_sentence[ind]].index(feature[0])
                if pos_sentence[ind][indice][1] in vb:
                    count_verb += 1
                    include = False
                   
            else:
                for term in [row[0] for row in pos_sentence[ind]]:
                    indice = [row[0] for row in pos_sentence[ind]].index(term)
                    if wordnet_lemmatizer.lemmatize(term,'v') == feature[0] and pos_sentence[ind][indice][1] in vb:
                        count_verb += 1
                        include = False
            ...
            for word in words:
                if word in [row[0] for row in pos_sentence[ind]]:
                    indice = [row[0] for row in pos_sentence[ind]].index(word)
                    if pos_sentence[ind][indice][1] in vb:
                        count_verb += 1
                        include = False
                        break
        ...
        if wordnet_lemmatizer.lemmatize(feature[0]) not in canon_feature and include == True:
            canon_feature.append(wordnet_lemmatizer.lemmatize(feature[0]))
...
for item in canon_feature:
    if ' ' in item:
        ...
        item = item.split(' ')
        for word in item:
            if word not in canon_phrase:
                canon_phrase.append(word)
...
for item in canon_output:
    if item not in canon_feature and item not in canon_phrase:
        confusion[1][0] += 1
        print item, canon_freq[wordnet_lemmatizer.lemmatize(item)],
