...
#                        if rel in dep and rel_j in dep:
                        if rel == rel_j or (rel in mods and rel_j in mods) or (rel in sobj[:-1] and rel_j == sobj[-1]) or (rel == sobj[-1] and rel_j in sobj[:-1]):
                            ...
                            if target_2_pos in jj and target_2 not in review_opinion[ind] and target_2 not in punc:
                                ...
                                if target_2 not in opinion_freq.keys():
                                    opinion_freq[target_2] = 1
                                else:
                                    opinion_freq[target_2] += 1
            ...
            if rel == 'conj' and dep_indice[0] != 'E' and indice[0] != 'E':
                ...
                if pos_sentence[ind][int(dep_indice) - 1][0] in processed_sentiwords:
                    ...
                    if pos_sentence[ind][int(indice) - 1][1] in jj and target not in review_opinion[ind] and target not in punc:
                        ...
                        if target not in opinion_freq.keys():
                            opinion_freq[target] = 1
                        else:
                            opinion_freq[target] += 1
...
feature_dic = cPickle.dump(review_feature, open('feature_dic_3', 'w'))  
opinion_dic = cPickle.dump(review_opinion, open('opinion_dic_3', 'w')) 

cPickle.dump(feature_freq, open('feature_freq_3', 'w')) 
cPickle.dump(opinion_freq, open('opinion_freq_3', 'w'))  
cPickle.dump(target_pair, open('target_pair_3', 'w'))    
cPickle.dump(conj_set, open('conjunction_set_3', 'w'))            

out_feature = open('feat_output_combine_3', 'w')
out_opinion = open('opinion_output_combine_3', 'w')
