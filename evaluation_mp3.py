...
for ind, line in enumerate(mp3_review):
    line = line.split('##')
    ...
    for feature in features:
        ...
        if ' ' not in feature[0]:
            if feature[0] in [row[0] for row in pos_sentence[1489 + ind]]:
                indice = [row[0] for row in pos_sentence[1489 + ind]].index(feature[0])
                if pos_sentence[1489 + ind][indice][1] in vb:
                    count_verb += 1
                    include = False
                   
            else:
                for term in [row[0] for row in pos_sentence[1489 + ind]]:
                    indice = [row[0] for row in pos_sentence[1489 + ind]].index(term)
                    if wordnet_lemmatizer.lemmatize(term, 'v') == feature[0] and pos_sentence[1489 + ind][indice][1] in vb:
                        count_verb += 1
                        include = False
            ...
            for word in words:
                if word in [row[0] for row in pos_sentence[1489 + ind]]:
                    indice = [row[0] for row in pos_sentence[1489 + ind]].index(word)
                    if pos_sentence[1489 + ind][indice][1] in vb:
                        count_verb += 1
                        include = False
                        break
        ...
        if wordnet_lemmatizer.lemmatize(feature[0]) not in mp3_feature and include == True:
            mp3_feature.append(wordnet_lemmatizer.lemmatize(feature[0]))
...
phrase_count = 0 
phrase_set = []          
for item in mp3_feature:
    if ' ' in item:
        phrase_count += 1
        phrase_set.append(item)
        item = item.split(' ')
        for word in item:
            if word not in mp3_phrase:
                mp3_phrase.append(word)
...
for line in mp3_predict:
    if line != '':
        for item in line:
            item = wordnet_lemmatizer.lemmatize(item)
            if item not in mp3_output:
                mp3_output.append(item)
'''
...
for item in mp3_output:
    if item not in mp3_feature and item not in mp3_phrase:
        confusion[1][0] += 1
        print item, mp3_freq[wordnet_lemmatizer.lemmatize(item)],
