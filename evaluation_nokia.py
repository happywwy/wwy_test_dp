...
for ind, line in enumerate(nokia_review):
    line = line.split('##')
    ...
    for feature in features:
        ...
        if ' ' not in feature[0]:
            if feature[0] in [row[0] for row in pos_sentence[943+ind]]:
                indice = [row[0] for row in pos_sentence[943+ind]].index(feature[0])
                if pos_sentence[943+ind][indice][1] in vb:
                    count_verb += 1
                    include = False
                   
            else:
                for term in [row[0] for row in pos_sentence[943+ind]]:
                    indice = [row[0] for row in pos_sentence[943+ind]].index(term)
                    if wordnet_lemmatizer.lemmatize(term,'v') == feature[0] and pos_sentence[943+ind][indice][1] in vb:
                        count_verb += 1
                        include = False
            ...
            for word in words:
                if word in [row[0] for row in pos_sentence[943+ind]]:
                    indice = [row[0] for row in pos_sentence[943+ind]].index(word)
                    if pos_sentence[943+ind][indice][1] in vb:
                        count_verb += 1
                        include = False
                        break
        ...
        if wordnet_lemmatizer.lemmatize(feature[0]) not in nokia_feature and include == True:
            nokia_feature.append(wordnet_lemmatizer.lemmatize(feature[0]))
...
phrase_count = 0 
phrase_set = []          
for item in nokia_feature:
    if ' ' in item:
        phrase_count += 1
        phrase_set.append(item)
        item = item.split(' ')
        for word in item:
            if word not in nokia_phrase:
                nokia_phrase.append(word)
...
for line in nokia_predict:
    if line != '':
        for item in line:
            item = wordnet_lemmatizer.lemmatize(item)
            if item not in nokia_output:
                nokia_output.append(item)
'''
...
for item in nokia_output:
    if item not in nokia_feature and item not in nokia_phrase:
        confusion[1][0] += 1
        print item, nokia_freq[wordnet_lemmatizer.lemmatize(item)],
